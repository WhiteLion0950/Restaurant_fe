...
class Categories extends React.Component {
  ...
  fetchData() {
    //la invoco per reperire i dati quando viene creato il componente a cui servono i dati
    //settare lo state
    this.setState({
      //mettere il primo stato loading a true
      isLoading: true,
    });
                ...
                {this.state.data.map((current, idx) => {
                  return (
                    <Col md={4} key={"card_loading_" + idx}>
                      <Card
                        callback={() => {
                          this.props.history.push(`/categories/${current.id}`);
                        }}
